...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a numbered chat archive under ./.local/docs/chat-archive."
    )
    parser.add_argument("--base-dir", required=True, help="Workspace root for the archive")
    ...
    parser.add_argument("--project", required=True, help="Project name or none")
    parser.add_argument(
        "--source-range", required=True, help="Conversation range covered by this archive"
    )
    parser.add_argument(
        "--tags",
        required=True,
    ...
    parser.add_argument(
        "--body-file", required=True, help="Path to a Markdown file containing the archive body"
    )
...
def next_number(archive_dir: Path) -> int:
    highest = 0
...
def main() -> int:
    ...
    archive_dir = base_dir / ".local" / "docs" / "chat-archive"
    ...
    number = f"{next_number(archive_dir):05d}"
    note_path = archive_dir / f"{number}_{slugify(description)}.md"

    frontmatter = build_frontmatter(number, title, project, source_range, tags, description)
